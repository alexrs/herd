...
def check_activations(model, tokenizer, dataset, experts, experts_dir):
    ...
    for sample in samples:
        ...
        # )
        t = activation['lora_router']
        if t.shape == torch.Size([1, 1, 5]):
            activations.append(t)
        else:  # assuming that only the middle dimension can vary
            averaged_tensor = torch.mean(t, dim=1, keepdim=True)
            activations.append(averaged_tensor)

    print(activations)
    summed_tensor = torch.sum(torch.stack(activations), dim=0)

    print(summed_tensor.squeeze())



def test_molora(model_values, path_values, config, experts) -> None:
    experts_path = os.path.join(path_values.output_dir, "molora")
    model_path = os.path.join(experts_path, "router_top_2")
