...
class MultiloraModel(PeftModel):
    """
    ...
    Args:
        ...
        peft_strategy (str): The PEFT strategy to be used. Either `lora` or `ia3`.
    ...
    def __init__(self, model, adapters, peft_strategy, adapter_names, router):
        # Build PerftConfig from the first adapter
        config = PeftConfig.from_pretrained(os.path.join(adapters, peft_strategy, adapter_names[0]))
        super().__init__(model, config, adapter_names[0])

        # Load the other adapters
        for adapter_name in adapter_names[1:]:
            self.load_adapter(os.path.join(adapters, peft_strategy, adapter_name), adapter_name)

        self.router = router

    def generate(self, prompt: str, top: int = 1, **kwargs):
        logger.debug("Generate inside MultiloraModel")
